...
class DashboardForm : Form
{
    ...
    void BuildUI()
    {
        ...
        btnPostCalc.FlatAppearance.BorderSize = 0;
        btnPostCalc.Click += async (s, e) =>
        {
            var results = "";

            async Task TryVerbose(string label, object payload)
            {
                try
                {
                    ...
                    var response = await ApiClient.Http.PostAsync("att/api/calculation/", content);
                    var body = await response.Content.ReadAsStringAsync();
                    results += $"\n=== {label} ({(int)response.StatusCode}) ===\n{body}\n";
                }
                catch (Exception ex) { results += $"{label}: ERR {ex.Message}\n"; }
            }

            await TryVerbose("emp_code + id + dates", new { emp_code = "7", id = 7, start_date = "2026-05-18", end_date = "2026-05-18" });
            await TryVerbose("emp_code + employees + dates", new { emp_code = "7", employees = new[] { 7 }, start_date = "2026-05-18", end_date = "2026-05-18" });
            await TryVerbose("emp_code only", new { emp_code = "7" });
            await TryVerbose("emp_code + start_date only", new { emp_code = "7", start_date = "2026-05-18" });
            await TryVerbose("emp_code + end_date only", new { emp_code = "7", end_date = "2026-05-18" });

            MessageBox.Show(results, "Calc field name test");
        };
        toolbar.Controls.Add(btnPostCalc);
...
class ExportDialog : Form
{
    ...
    void WriteSheetHeader(IXLWorksheet ws, string deptName, string[] headers, int totalCols)
    {
        ...
        for (int c = 0; c < headers.Length; c++)
        {
            ...
            // Columns A–H: punch data (blue)
            // Columns I–L: API attendance fields (teal)
            // Columns M–S: payroll computed fields (dark orange)
            if (c < 8)
    ...
    void WriteDataRow(IXLWorksheet ws, int row, DailyRecord rec, int totalCols)
    {
        ...
        //  TOTAL REG HRS, TOTAL OT HRS)
        for (int c = 9; c <= totalCols; c++)
            ws.Cell(row, c).Value = "";

        // Clean white background, no fill
        ...
        // Center time/date and payroll columns
        for (int c = 4; c <= totalCols; c++)
            ws.Cell(row, c).Style.Alignment.Horizontal =
                XLAlignmentHorizontalValues.Center;

        // Light yellow tint on payroll columns so they stand out as "fill me in"
